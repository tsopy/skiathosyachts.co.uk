...
if (file_exists($bookedFile)) {
    $bookedDates = json_decode(file_get_contents($bookedFile), true);
}

if (in_array($date, $bookedDates)) {
    unlink($pendingFile);
    die("⚠️ Η ημερομηνία {$date} είναι ήδη δεσμευμένη!");
}

$bookedDates[] = $date;
file_put_contents($bookedFile, json_encode($bookedDates));

$to = $email;
$subject = "✅ Η κράτησή σας στο THEMIS IV εγκρίθηκε!";
$message = "
<html>
<head><meta charset='UTF-8'><title>Κράτηση Εγκρίθηκε</title></head>
<body>
    <h2>Αγαπητέ/ή {$name},</h2>
    <p>Η κράτησή σας για την ημερομηνία <strong>{$date}</strong> εγκρίθηκε!</p>
    <p>Σας περιμένουμε για μια αξέχαστη εμπειρία.</p>
...
<!DOCTYPE html>
<html lang="el">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Κράτηση Εγκρίθηκε</title>
...
<body>
<div class="container">
    <h1>✅ Η κράτηση εγκρίθηκε!</h1>
    <p>Η ημερομηνία <strong><?php echo $date; ?></strong> δεσμεύθηκε.</p>
